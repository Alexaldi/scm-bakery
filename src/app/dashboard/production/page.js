    ...
    <div>
      ...
      <FormModal open={createOpen} title="Buat Order Produksi" onClose={() => setCreateOpen(false)}>
        <form onSubmit={submitCreate} className="grid gap-4 md:grid-cols-2">
          <label className="text-sm font-medium text-gray-700">Sumber Peramalan<select value={form.forecastId} onChange={(event) => changeForecast(event.target.value)} className={inputClass}><option value="">Tanpa peramalan</option>{approvedForecasts.map((forecast) => { const product = products.find((item) => item.id === forecast.productId); return <option key={forecast.id} value={forecast.id}>{product?.name} - {formatPeriod(forecast.period)}</option>; })}</select></label>
